...
import { 
  Users, Activity, PlusCircle, FileSpreadsheet, Save, Trash2, Home, Upload, Eye, X, Lock, Unlock, AlertTriangle, BarChart2, CloudIcon
} from 'lucide-react';
import { initializeApp } from 'firebase/app';
import { getAuth, signInAnonymously, onAuthStateChanged, signInWithCustomToken } from 'firebase/auth';
import { getFirestore, collection, doc, setDoc, deleteDoc, onSnapshot } from 'firebase/firestore';

// --- Firebase Initialization (Fallbacks gracefully if not provided) ---
...
const getQuestions = (subject) => {
  ...
  // Domain: 關節活動度 (ROM) - 調整順序為: 右主內 -> 右被內 -> 右主外 -> 右被外 -> 左側...
  q.push(
        ...
        <div className="max-w-xl mx-auto space-y-8 animate-in fade-in duration-300 pb-8">
          ...
          {currentDomainQuestions.map((q) => (
            <div key={q.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
              ...
              {/* 極簡版 K-Pull 純上傳介面 - 支援單檔解析包含三次試驗的 Mean/SD 即時預覽 */}
              {q.type === 'kpull_upload' && (
...
export default function App() {
  ...
  // 2. 掛載 jStat 統計套件與讀取資料 (Firestore / LocalStorage)
  useEffect(() => {
    // 載入 jStat 套件做 ANOVA 分析
    if (!window.jStat) {
